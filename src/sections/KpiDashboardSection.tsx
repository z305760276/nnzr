      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
        {KPI_CARDS.map((card, i) => {
          ...
          const pct = card.unit === '%' ? parseInt(card.value)
            : card.unit === 'h' ? Math.min(Math.max((48 - parseInt(card.value)) / 48 * 100, 8), 100)
            : 0;
          ...
          return (
            <div key={i} className="kpi-card kpi-card-liquid p-5 flex flex-col"
              ...
              <div className="mt-4 pt-3 flex items-center justify-between"
                ...
                <span className="text-[10px] font-medium" style={{ color: card.accent }}>● 当前值 {card.value}{card.unit}</span>
              </div>
            </div>
          );
        })}
      ...
      <section>
        ...
        <AccordionGroup variant="kpi" className="space-y-3">
          {kpiDetails.map((kpi, i) => {
            const pct = kpi.unit === '%' ? parseInt(kpi.current)
              : kpi.unit === 'h' ? Math.min(Math.max((48 - parseInt(kpi.current)) / 48 * 100, 8), 100)
              : 0;
