...
function OrgChartNode({ data, level, onNodeClick }: { data: any; level: number; onNodeClick: (node: OrgNode) => void }) {
  ...
  const colors = LEVEL_COLORS[level] || LEVEL_COLORS[5]
...
function OrgGraphSection() {
  ...
  const [modalData, setModalData] = useState<{ title: string; items: string[]; color: string } | null>(null)
  ...
  const renderModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={() => setModalData(null)}>
        ...
        <div
          className="relative w-full max-w-lg max-h-[70vh] rounded-2xl overflow-hidden flex flex-col"
          style={{
            background: 'var(--kpi-glass-bg-strong, #1a1a2e)',
            backdropFilter: 'blur(24px) saturate(180%)',
            border: '1px solid var(--kpi-glass-border, rgba(255,255,255,0.08))',
            boxShadow: '0 24px 64px rgba(0,0,0,0.5)',
          }}
          onClick={(e) => e.stopPropagation()}
        >
          <div className="flex items-center justify-between px-5 py-4 shrink-0"
            style={{ borderBottom: '1px solid var(--kpi-divider, rgba(255,255,255,0.06))' }}>
            <div className="flex items-center gap-2.5">
              <div className="size-8 rounded-lg flex items-center justify-center" style={{ background: `${modalData.color}20` }}>
                {modalData.title.includes('安全') ? (
                  <Shield className="size-4" style={{ color: modalData.color }} />
                ) : (
                  <FileText className="size-4" style={{ color: modalData.color }} />
                )}
              </div>
              <div>
                <h3 className="text-sm font-bold text-foreground">{modalData.title}</h3>
                <p className="text-[10px] text-muted-foreground">共 {modalData.items.length} 条</p>
              </div>
            </div>
            <button
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4">
            <ol className="space-y-2 list-decimal list-inside">
              {modalData.items.map((item, i) => (
                <li key={i} className="text-xs text-muted-foreground leading-relaxed rounded-lg px-3 py-2"
                  style={{ background: `${modalData.color}08` }}>
          ...
          <div className="px-5 py-3 shrink-0 flex justify-end"
            style={{ borderTop: '1px solid var(--kpi-divider, rgba(255,255,255,0.06))' }}>
            <span className="text-[10px] text-muted-foreground/60">{modalData.items.length} 条全部显示</span>
          </div>
        </div>
      </div>
    )
        ...
        <div className="grid grid-cols-1 md:grid-cols-2">
          <div className="p-5 space-y-3">
            <h4 className="text-xs font-semibold text-primary flex items-center gap-1.5">
              ...
              <span className="font-normal text-muted-foreground/60">（{n.responsibilities.length > 5 ? `展示5条/共${n.responsibilities.length}条` : `共${n.responsibilities.length}条`}）</span>
            </h4>
            <ol className="space-y-2 list-decimal list-inside">
              {n.responsibilities.slice(0, 5).map((r, i) => (
            ...
            {n.responsibilities.length > 5 && (
              <button
                onClick={() => setModalData({ title: `岗位职责 - ${n.title}`, items: n.responsibilities, color: 'var(--primary, #3B82F6)' })}
                className="w-full mt-2 text-xs font-medium py-2 rounded-lg transition-all hover:bg-accent flex items-center justify-center gap-1"
          ...
          <div className="p-5 space-y-3 border-l border-border">
            <h4 className="text-xs font-semibold flex items-center gap-1.5" style={{ color: '#DC2626' }}>
              ...
              <span className="font-normal text-muted-foreground/60">（{n.safetyDuties.length > 5 ? `展示5条/共${n.safetyDuties.length}条` : `共${n.safetyDuties.length}条`}）</span>
            </h4>
            <ol className="space-y-2 list-decimal list-inside">
              {n.safetyDuties.slice(0, 5).map((d, i) => (
            ...
            {n.safetyDuties.length > 5 && (
              <button
                onClick={() => setModalData({ title: `安全职责 - ${n.title}`, items: n.safetyDuties, color: '#DC2626' })}
                className="w-full mt-2 text-xs font-medium py-2 rounded-lg transition-all hover:bg-red-50 dark:hover:bg-red-950/30 flex items-center justify-center gap-1"
                style={{ color: '#DC2626' }}
              >
                展开全部
              </button>
